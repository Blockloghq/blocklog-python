...
def main() -> None:
    ...
    with client.session(agent_id="langchain-demo", source="langchain", workflow_id=uuid4()):
        run_id = uuid4()
        handler.on_chain_start({"name": "demo-chain"}, {"question": "What happened?"}, run_id=run_id)
        handler.on_tool_start({"name": "search_docs"}, "find refund policy", run_id=uuid4(), parent_run_id=run_id)
